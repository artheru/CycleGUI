...
namespace Annotator.RenderTypes
{
    internal class CylinderRender : BasicRender
    {
        ...
        {
            pb.DragFloat($"height {Name}", ref Height, 0.01f, 0.1f, 100f);
            pb.DragFloat($"top radius {Name}", ref TopRadius, 0.01f, 0.1f, 100f);
            pb.CheckBox($"same bottom & top radius {Name}", ref TopBottomSame);
            if (!TopBottomSame) pb.DragFloat($"bottom radius {Name}", ref BottomRadius, 0.01f, 0.1f, 100f);
