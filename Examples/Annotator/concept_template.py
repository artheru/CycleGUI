...
class Multilevel_Body(ConceptTemplate):
    def __init__(self, 
                 num_levels=[2],  # Default to 1 level
                 level_1_size=[0.21, 0.86, 0.21],  # Default sizes
                 level_2_size=[0.045, 0.52],
                 level_3_size=[0.04, 0.215],
                 level_4_size=[0.5, 0.3],
                 position=[0, 0, 0], 
                 rotation=[0, 0, 0]):
        ...
        if num_levels[0] > 4:  # Ensure no more than 4 levels
            num_levels = [4]
        # Record Parameters
...
class Cylindrical_Lid(ConceptTemplate):
    def __init__(self, 
                 outer_size=[0.04, 0.04, 0.22],  # Default outer dimensions [radius_bottom, radius_top, height]
                 inner_size=[0.035, 0.0355, 0.21],  # Default inner dimensions [radius_bottom, radius_top, height]
                 position=[0, 0, 0], 
                 rotation=[0, 0, 0]):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
