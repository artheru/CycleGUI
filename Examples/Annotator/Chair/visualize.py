...
if __name__ == "__main__":
    ...
    for data in data_list:
        ...
        conceptualization_rendering.paint_uniform_color([0, 0, 1])  # Blue: Initial conceptualization
        o3d.visualization.draw_geometries([conceptualization_rendering])
        if has_object_model:
            print("Performing deformable alignment...")
            # Prepare vertices for alignment
            A = np.asarray(conceptualization_rendering.vertices)
            B = np.asarray(source_mesh.vertices)
            neighbors = compute_neighbors(A, max_distance=0.1)

            # Deformable alignment
            alpha = 0.1
            iterations = 200
            learning_rate = 0.005
            D = gradient_descent(A, B, neighbors, alpha, iterations, learning_rate)

            # Apply deformation to conceptualization_rendering
            A_deformed = A + D
            conceptualization_rendering.vertices = o3d.utility.Vector3dVector(A_deformed)
        # Get arrow (for better visualization)
        if has_object_model:
            arrow_mesh = draw_arrow(mesh_scale * 0.5)

        conceptualization_rendering.paint_uniform_color([0, 1, 0])  # Blue: Initial conceptualization
        source_mesh.paint_uniform_color([1, 0, 0])  # Red: Source (reference) mesh
        o3d.visualization.draw_geometries([conceptualization_rendering])
        o3d.visualization.draw_geometries([source_mesh])
