...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    const float inner_spacing_explicit = (pad_inner_x && (flags & ImGuiTableFlags_BordersInnerV) == 0) ? g.Style.imstyleCellPadding.x : 0.0f;
    const float inner_padding_explicit = (pad_inner_x && (flags & ImGuiTableFlags_BordersInnerV) != 0) ? g.Style.imstyleCellPadding.x : 0.0f;
    table->CellSpacingX1 = inner_spacing_explicit + inner_spacing_for_border;
    table->CellSpacingX2 = inner_spacing_explicit;
    table->CellPaddingX = inner_padding_explicit;
    table->CellPaddingY = g.Style.imstyleCellPadding.y;

    const float outer_padding_for_border = (flags & ImGuiTableFlags_BordersOuterV) ? TABLE_BORDER_SIZE : 0.0f;
    const float outer_padding_explicit = pad_outer_x ? g.Style.imstyleCellPadding.x : 0.0f;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    table->MinColumnWidth = ImMax(1.0f, g.Style.imstyleFramePadding.x * 1.0f); // g.Style.ColumnsMinSpacing; // FIXME-TABLE
    ...
    const float width_removed = (table->HasScrollbarYPrev && !table->InnerWindow->ScrollbarY) ? g.Style.imstyleScrollbarSize : 0.0f; // To synchronize decoration width of synched tables with mismatching scrollbar state (#5920)
...
float ImGui::TableGetHeaderRowHeight()
{
    ...
    row_height += GetStyle().imstyleCellPadding.y * 2.0f;
    return row_height;
}
...
void ImGui::TableHeader(const char* label)
{
    ...
    {
        w_arrow = ImFloor(g.FontSize * ARROW_SCALE + g.Style.imstyleFramePadding.x);
        if (column->SortOrder > 0)
        {
            ImFormatString(sort_order_suf, IM_ARRAYSIZE(sort_order_suf), "%d", column->SortOrder + 1);
            w_sort_text = g.Style.imstyleItemInnerSpacing.x + CalcTextSize(sort_order_suf).x;
        }
    }
    ...
    ImRect bb(cell_r.Min.x, cell_r.Min.y, cell_r.Max.x, ImMax(cell_r.Max.y, cell_r.Min.y + label_height + g.Style.imstyleCellPadding.y * 2.0f));
    ...
    window->DC.CursorPos.y -= g.Style.imstyleItemSpacing.y * 0.5f;
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        if (column->SortOrder != -1)
        {
            ...
            if (column->SortOrder > 0)
            {
                ...
                RenderText(ImVec2(x + g.Style.imstyleItemInnerSpacing.x, y), sort_order_suf);
                PopStyleColor();
                x += w_sort_text;
            }
    ...
    RenderTextEllipsis(window->DrawList, label_pos, ImVec2(ellipsis_max, label_pos.y + label_height + g.Style.imstyleFramePadding.y), ellipsis_max, ellipsis_max, label, label_end, &label_size);
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    x = ImMax(x, ImGui::GetColumnOffset(column_index - 1) + g.Style.imstyleColumnsMinSpacing);
    if ((columns->Flags & ImGuiOldColumnFlags_NoPreserveWidths))
        x = ImMin(x, ImGui::GetColumnOffset(column_index + 1) - g.Style.imstyleColumnsMinSpacing);
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    if (!(columns->Flags & ImGuiOldColumnFlags_NoForceWithinWindow))
        offset = ImMin(offset, columns->OffMaxX - g.Style.imstyleColumnsMinSpacing * (columns->Count - column_index));
    columns->Columns[column_index].OffsetNorm = GetColumnNormFromOffset(columns, offset - columns->OffMinX);

    if (preserve_width)
        SetColumnOffset(column_index + 1, offset + ImMax(g.Style.imstyleColumnsMinSpacing, width));
}
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    const float column_padding = g.Style.imstyleItemSpacing.x;
