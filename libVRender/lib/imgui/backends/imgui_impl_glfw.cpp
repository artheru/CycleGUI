...
//#define GLFW_HAS_VULKAN                 (GLFW_VERSION_COMBINED >= 3200) // 3.2+ glfwCreateWindowSurface //patch.
...
{
    // just no joystick. totally rework joystick's logic in cyclegui.
    return;
    ImGuiIO& io = ImGui::GetIO();
...
static bool ImGui_ImplGlfw_GetWindowMinimized(ImGuiViewport* viewport)
{
    ...
    // Check if the window is minimized using GLFW
    if (glfwGetWindowAttrib(vd->Window, GLFW_ICONIFIED) != 0)
        return true;

    // Check if the window is visible using the Windows API
