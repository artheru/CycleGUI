...
struct IMGUI_API ImGuiInputTextState
{
    ...
    bool                    ExternEdited;           // edited externally (e.g. via platform IME bridge)
    ImGuiInputTextFlags     Flags;                  // copy of InputText() flags. may be used to check if e.g. ImGuiInputTextFlags_Password is set.
    ImRect                  FrameBB;                // bounding box of the frame for positioning platform IME helpers
...
public:
    ...
    float       TitleBarHeight() const
    {
        ...
        return (Flags & ImGuiWindowFlags_NoTitleBar) ? 0.0f : CalcFontSize() + g.Style.imstyleFramePadding.y * 2.0f;
    }
    ...
    float       MenuBarHeight() const
    {
        ImGuiContext& g = *Ctx;
        auto window = g.CurrentWindow;
        return (Flags & ImGuiWindowFlags_MenuBar) ? DC.MenuBarOffset.y + CalcFontSize() + g.Style.imstyleFramePadding.y * 2.0f : 0.0f;
    }
