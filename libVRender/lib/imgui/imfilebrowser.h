...
inline void ImGui::FileBrowser::Display()
{
...
#ifdef _WIN32
    char path[1024];
    strcpy_s(path, (char*)pwd_.c_str());
    if (InputText("Path", path, 1024, ImGuiFileBrowserFlags_ConfirmOnEnter))
    {
    ...
    PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(3, 0));
...
struct SelectDirectory
{
    ImGui::FileBrowser fb = ImGui::FileBrowser(ImGuiFileBrowserFlags_SelectDirectory | ImGuiFileBrowserFlags_HideRegularFiles);
};
