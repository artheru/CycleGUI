...
namespace
{
	...
	void DrawManualSettingsTab(bool allowModify)
	{
		...
		ImGui::Checkbox("Render debug overlay", &ui.RenderDebug);
		ImGui::DragFloat("SSAO weight", &ssao_uniforms.weight, 0.05f, 0.0f, 5.0f);
		ImGui::DragFloat("SSAO sample radius", &ssao_uniforms.uSampleRadius, 0.1f, 0.0f, 50.0f);
		ImGui::DragFloat("SSAO bias", &ssao_uniforms.uBias, 0.01f, 0.0f, 1.0f);
		ImGui::DragFloat2("SSAO attenuation", ssao_uniforms.uAttenuation, 0.01f, 0.0f, 2.0f);
...
void ProcessWorkspace(disp_area_t disp_area, ImDrawList* dl, ImGuiViewport* viewport)
{
	...
	if (working_viewport == ui.viewports) {
		...
		if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayShort) && appStat && appStat[0] != '\0')
		{
			ImGui::SetTooltip("%s", appStat);
		}
